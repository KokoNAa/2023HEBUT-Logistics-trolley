...
void ST3_Move(int32_t step, uint32_t accel, uint32_t decel, int32_t speed)
{
    ...
    {					
			...
			motor_sta = 1;  																												// 电机为运动状态
			tim_count = TIM_GetCounter(TIM3);																				//获取计数值
			TIM_SetCompare2(TIM3,tim_count+srd3.step_delay/2);												//设置定时器比较值 
...
void speed1_decision()                                                         //中断执行函数
{
  ...
  __IO static uint16_t st1_last_accel_delay=0;                                    // 加速过程中最后一次延时（脉冲周期）. 
  __IO static uint32_t st1_step_count = 0; 																			  // 总移动步数计数器  
  __IO static int32_t st1_rest = 0;																								// 记录new_step_delay中的余数，提高下一步计算的精度  
		  ...
		  switch(srd1.run_state)
			{
				...
				st1_step_count++;
				srd1.accel_count++;
				new_step_delay = srd1.step_delay - (((2 *srd1.step_delay) + st1_rest)/(4 * srd1.accel_count + 1));//计算新(下)一步脉冲周期(时间间隔)
				st1_rest = ((2 * srd1.step_delay)+st1_rest)%(4 * srd1.accel_count + 1);					// 计算余数，下次计算补上余数，减少误差
				
					if(st1_step_count >= srd1.decel_start) 																	//检查是够应该开始减速
					{
					...
					{
						st1_last_accel_delay = new_step_delay;																//保存加速过程中最后一次延时（脉冲周期）
...
//		  }
          new_step_delay = srd1.min_delay;   																  // 使用min_delay（对应最大速度speed）				 
          if(st1_step_count >= srd1.decel_start)   																// 需要开始减速
		  {
            srd1.accel_count = srd1.decel_val;  																// 减速步数做为加速计数值
            new_step_delay = st1_last_accel_delay;																// 加阶段最后的延时做为减速阶段的起始延时(脉冲周期)
